...
class WooThemes_Sensei_Grading_Main extends WooThemes_Sensei_List_Table {
	...
	public function __construct ( $args = null ) {
		...
		$this->module_id = intval( $args['module_id'] );
	...
	function get_columns() {
		$columns = array(
			...
			'module' => __( 'Modules', 'woothemes-sensei' ),
			...
			'assesor' => __( 'Assesor', 'woothemes-sensei' ),
			'action' => '',
		);
	...
	public function prepare_items() {
		...
		if( $this->module_id ) {
			$activity_args['post_id'] = $this->module_id;
		}
	...
	protected function get_row_data( $item ) {
		...
		$module = get_the_terms( $item->comment_post_ID, 'module' );
		
		$allgroup = $wpdb->get_results( $wpdb->prepare( "SELECT group_id FROM wp_bp_groups_groupmeta where meta_key = 'bp_course_attached' and meta_value = %d", $course_id ) );

		foreach($allgroup as $groupids){
			
		
			foreach($groupids as $groupid){

			$group_ids = $wpdb->get_results( $wpdb->prepare( "SELECT group_id FROM wp_bp_groups_members where user_id = %d and group_id=%d",$item->user_id, $groupid ) );
		...
		if(is_array($group_ids)){
			...
			foreach($group_ids as $gid){
				...
				$user_info = get_userdata( groups_get_groupmeta( $gid->group_id, 'assesor', true));
				$assesor[] = $user_info ->display_name;
				$i++;
			}
			
			$assesor = join(',',$assesor);
			
			
		}else{
			$user_info = get_userdata( groups_get_groupmeta( $group_ids, 'assesor', true));
			$assesor = $user_info ->display_name;
	...
	public function data_table_header() {
		...
		echo '<div class="select-box">' . "\n";
			...
			echo '<select id="grading-module-options" data-placeholder="&larr; ' . __( 'Select a Module', 'woothemes-sensei' ) . '" name="grading_modules" class="chosen_select widefat">' . "\n";

				echo $woothemes_sensei->grading->modules_drop_down_html( $this->course_id);
		...
		if( $this->course_id ) {
			...
			echo '<div class="select-box reset-filter">' . "\n";
				...
				echo '<a class="button-secondary" href="' . esc_url( remove_query_arg( array( 'module_id', 'course_id' ) ) ) . '">' . __( 'Reset filter', 'woothemes-sensei' ) . '</a>' . "\n";
		...
		if( $this->module_id ) {
			$query_args['module_id'] = $this->module_id;
			// Restrict to a single lesson
			$count_args['post_id'] = $this->module_id;
		}
		if( $this->search ) {
