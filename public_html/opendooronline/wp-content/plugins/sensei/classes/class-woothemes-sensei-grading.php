...
//			add_action( 'sensei_grading_notices', array( $this, 'sensei_grading_notices' ) );
	...
	public function get_lessons_dropdown() {
		...
		$html = $this->modules_drop_down_html( $course_id );
    ...
    public function admin_process_grading_submission() {
		...
		//floyd code for - Historical Question/Answer/Feedback Tracking
		
		$user_grading_notes_history = get_user_meta( $user_id, '_grading_notes_history', true ); 
		
		
		if(!empty($user_grading_notes_history) && is_array($user_grading_notes_history)){
			
			$user_grading_notes_history[$question_id][date('Y-m-d H:i:s')] = $question_feedback;
		...
		update_user_meta($user_id, '_grading_notes_history', $user_grading_notes_history);			
		
		
		$user_answer_content 		= $woothemes_sensei->quiz->get_user_question_answer( $quiz_lesson_id, $question_id, $user_id );
		$user_answer_history		= get_user_meta( $user_id, '_grading_answer_history', true ); 
		
		if(!empty($user_answer_history) && is_array($user_answer_history)){
			
			$user_answer_history[$question_id][date('Y-m-d H:i:s')] = $user_answer_content;
		...
		update_user_meta($user_id, '_grading_answer_history', $user_answer_history);	
...
/* 				echo "<pre>";
				print_r(get_user_meta($user_id,'_grading_notes_history',true));
				echo "</pre>"; */
				
				
				//using the quiz id, get the module id.
				$lesson_id = $woothemes_sensei->quiz->get_lesson_id( $quiz_id );
				$course_id = get_post_meta( $lesson_id, '_lesson_course', true );
				$module = get_the_terms($lesson_id,'module');										
				$module_id = $module[0]->term_id;
				$user_info = get_userdata($user_id);
				$user_email = $user_info->user_email;
				
				$results = $wpdb->get_results( 'SELECT Id FROM _isContact WHERE meta_field = "Email" AND meta_value = "'.$user_email.'"', OBJECT );
				$contact = $results[0];
				 $cid = $contact->Id;
				//$shortcode = "[i4w_achievegoal cid='".$cid."' callname='make".$module_id."online']";				
				//echo do_shortcode($shortcode);
				
				//create a function that will process infusionft apit
				call_infusion($cid,$module_id);	
				
				
				//Need to get all the lessons in the module, and retrieve their marks. If all 100%, then we tag.
				$module_progress = WooThemes_Sensei_Grading::calculate_user_module_progress_custom($user_id, $module_id, $course_id);
				$totally_competant = 'No';
				if($module_progress < 100){
					$totally_competant = 'No';
				}
				else {
					$totally_competant = 'Yes';
					//add infusionsoft tag
					call_infusion_complet_module($cid,$module_id);	
					
				}
				$msg = array(
					'updated',
					__( 'Quiz Graded Successfully! '.$totally_competant. ' '.$module_progress.'%', 'woothemes-sensei' ),
				);
			}
			?>
