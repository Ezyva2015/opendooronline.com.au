 ...
 class WooThemes_Sensei_Quiz {
	...
	*/
	...
	public function user_quiz_submit_listener() {
		...
		echo count($quiz_answers)." count";
		
		$module_data = get_the_terms( $lesson_id, 'module' );
		
		
		
		// check if the the file is existing and the answer is just one
		if(!empty($_FILES) and count($quiz_answers)==1){
			

			$moduleid = $module_data[0]->term_id;
			...
			$lessons = get_posts(array(
			  ...
			  'tax_query' => array(
				array(
				  'taxonomy' => 'module',
				  'field' => 'id',
				  'terms' => $moduleid, // Where term_id of Term 1 is "1".
				  'include_children' => false
				)
			  )
			));	

			//loop all the lessons from the module
			foreach($lessons as $lesson){
								
				//echo $lesson->post_title.'------'.$lesson->ID.'<br />';
				
				$meta_lesson = get_post_meta($lesson->ID);
				
				$quiz_id = $woothemes_sensei->lesson->lesson_quizzes($lesson->ID );
				 $lesson_quiz_questions = $woothemes_sensei->post_types->lesson->lesson_quiz_questions( $quiz_id );

				
				foreach($lesson_quiz_questions as $question){
					
					
					$modified_quiz_answers[$question->ID] = 'uploaded via file';
					
					
					
				}

				//save the quiz with the lessons
				self::submit_answers_for_grading( $modified_quiz_answers, $_FILES ,  $lesson->ID  , $current_user->ID );
				
				$modified_quiz_answers = "";
			}
        ...
        //check number of questions - if only one question, then check if that question is file upload.
        //if the quiz is single question file upload, then assume it is an "Offline Upload" and create empty submissions.
		//get lesson ids for the module, and create empty submissions using 'submit_answers_for_grading'.
		//e.g. lets say that lesson ids were $lesson_ids[] = 1,2,3,5,10
