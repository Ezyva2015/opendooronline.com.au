jQuery(document).ready( function($) {
		...
		jQuery.post(
			...
			function( response ) {
				...
				if ( '' != response ) {
					...
					jQuery( '#grading-module-options' ).empty().append( response );
					// Add Chosen to the drop down
					if ( jQuery( '#grading-module-options' ).exists() ) {
						// Show the Lessons label
						jQuery( '#grading-module-options-label' ).show();
						jQuery( '#grading-module-options' ).trigger("change");
					} // End If Statement
				} else {
					// Failed
				}
	 ...
	 * @access public
	 */
	jQuery( '#grading-module-options' ).on( 'change', '', function() {
		// Populate the Lessons select box
		
		var lessonId = jQuery(this).val();
		var courseId = jQuery( '#grading-course-options' ).val();
		var dataToPost = '';
	 	dataToPost += 'module_id' + '=' + lessonId;
	 	dataToPost += '&course_id' + '=' + courseId;
	...
	if ( jQuery( '#grading-module-options' ).exists() ) { jQuery( '#grading-module-options' ).select2(); }
